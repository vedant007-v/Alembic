...
class NegativeNetIncomeScreen extends StatefulWidget {
  @override
  _NegativeNetIncomeScreenState createState() => _NegativeNetIncomeScreenState();
}

class _NegativeNetIncomeScreenState extends State<NegativeNetIncomeScreen> {
  final int _limit = 20;
  ...
  void _showExpenseDetails(Map<String, dynamic> expense) async {
    // Already in your code — no changes needed
  }
        ...
        rows: _filteredData.map((row) {
          return DataRow(
            ...
            cells: _columnDefs.map((col) {
              final value = row[col.key];    return DataCell(
                Text(
                  (value is double)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Negative Net Income'),
        actions: [
          IconButton(
            ...
            onPressed: () => ExportUtils.exportToExcel(
              ...
              fileNamePrefix: 'Negative_Net_Income',
            ),
